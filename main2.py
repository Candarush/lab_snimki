...
def get_pix_from_coord(lon, lat):
    coeff_lon = (lon - COORD_UL[0]) / (COORD_UR[0] - COORD_UL[0])
    coeff_lat = (lat - COORD_UL[1]) / (COORD_LL[1] - COORD_UL[1])
    print(coeff_lon, coeff_lat)
    return (PIXEL_UL[0] + (PIXEL_UR[0] -  PIXEL_UL[0]) * coeff_lon, PIXEL_UL[1] + (PIXEL_LL[1] -  PIXEL_UL[1]) * coeff_lat)
...
if __name__ == '__main__':
    ...
    test = get_pix_from_coord(-123.26522827148439, 49.26455606028097)
    print(test)
    draw_dot(test[0], test[1], (0, 255, 0))
    show_image()
