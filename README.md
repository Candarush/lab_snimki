...
1. Открываем изображения Band 3 и Band 4, они содержат пиксели со значениями usigned char - целые числа от 0 до 255.
...
4. После этого можем посчитать коэффициент отражения R по формуле:  
   ...
   (считается по формуле: <strong><em>d = 1 − 0.01672 ∗ cos(0.9856 ∗ (day − 4))</strong></em>, где <strong><em>day</strong></em> - номер дня в году),  
...
Использование трансформации для перевода значений пикселей в коэффициенты отражения позволяет получить более контрастные изображения NDVI, что в свою очередь упрощает задачу анализа снимков.
...
2. Переводим матрицу в серое изображение, для этого:  
   ...
   Умножаем на 128 и переводим в тип unsigned char, чтобы получить диапазон целых чисел от 0 до 256.  
